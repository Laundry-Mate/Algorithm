...
def washing_algorithm():
    ...
    if num_clusters >= 2:
        ...
        dehydration_0 = dataset[dataset['dehydration_type'].isin(['None'])]
        ...
        if dehydration_1.shape[0] > 0:
            result.append({
                'dehydration': 'Light',
                'average_wash_temperature': avg_water_temperature,
                'indices': indices
            })
    ...
    if dry_cleaning.shape[0] > 0:
        indices = dry_cleaning.index.tolist()
        result.append({
            'basket': 'dry_cleaning',
            'dehydration': 'None',
            'indices': indices
    ...
    if handwash_only.shape[0] > 0:
        indices = handwash_only.index.tolist()
        result.append({
